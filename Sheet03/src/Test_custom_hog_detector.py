...
class Custom_Hog_Detector:
    ...
    def detection_scalePyramid(self,img_path,hog):
        ...
        for i,resized_img in enumerate(self.scale_pyramid(image,self.scale,self.win_size)):
            for(x,y,cur_window) in self.sliding_win(image,self.window_stride,self.win_size):
                ...
                # hog_feats= np.float32(hog_feats)
        ...
        reduced_detections = self.nms(detections,overlap_threshold)
    ...
    def non_max_suppression(self,boxes, threshold,pred_vals):
        ...
        while(len(idxs)>0):
            ...
            for pos in range(0,last):
                ...
                xx2 = min(x_2[i],x_2[j])
                yy2 = min(y_2[i],y_2[j])
                # print(f"xx1 = max(x_1[i]->{x_1[i]},x_1[j]->{x_1[j]}): {xx1} ")
                # print(f"xx2 = min(x_2[i]->{x_2[i]},x_2[j]->{x_2[j]}): {xx2} ")
                # print(f"yy1 = max(y_1[i]->{y_1[i]},y_1[j]->{y_1[j]}): {yy1} ")
                # print(f"yy2 = min(y_2[i]->{y_2[i]},y_2[j]->{y_2[j]}): {yy2} ")
                # print(f"j=idxs[last]: idxs{last}= {j}")
                # print(f"xx1: {xx1}, xx2:{xx2}, yy1: {yy1}, yy2:{yy2}")

                #Compute the height and width of the bounding box
                w = max(0,xx2-xx1)
                h = max(0,yy2-yy1)
                # print(f"w = max(0,xx2({xx2})-xx1({xx1})): {w} ")
                # print(f"h = max(0,yy2({yy2})-yy1({yy1})): {h} ")
                #
    ...
    def nms(self,boxes,threshold):
        ...
        while len(idxs) > 0:
            # Get the last index in the indexes list and add the index value to the list of picked indexes
            last = len(idxs) - 1
            i = idxs[last]
            picked_idxs.append(i)
